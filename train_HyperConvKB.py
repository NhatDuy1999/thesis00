...
from HyperConvKB import HyperConvKB
import json
import os
import numpy as np

from argparse import ArgumentParser
parser = ArgumentParser("HyperConvKB")
...
out_dir = os.path.abspath(os.path.join("../runs_pytorch_HyperConvKB/"))
print("Writing to {}\n".format(out_dir))
...
if args.mode == "train":
    ...
    con.set_train_model(HyperConvKB)
    con.training_model()
    ...
    con.set_config_CNN(num_of_filters=args.num_of_filters, drop_prob=args.dropout, kernel_size=args.kernel_size)
    con.set_test_model(HyperConvKB, args.checkpoint_path)
    con.test()
